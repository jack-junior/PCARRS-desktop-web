...
# Récupérer le batch_name (défini dans l'app Shiny)
...
if (b_name != "") {
  ...
  report_out <- file.path(cfg$paths$reports, b_name, "reports_English")
} else {
...
generate_officer_report <- function(pid, data_full) {
  ...
  info_df <- data.frame(
    ...
    Val = c(
      ...
      ifelse(is.na(person$`Name of Participant`[1]), "N/A", as.character(person$`Name of Participant`[1])), # Le nom vient du merge
  ...
  # --- CHARTS ---
  # plot_data_steps <- data.frame(d = paste("Day", 1:7), s = steps_vec) %>%
  #   filter(!is.na(s) & s > 0) 
  # 
  ...
  # plot_data_sleep <- data.frame(n = paste("Night", 1:7), s = sleep_vec) %>%
  #   filter(!is.na(s) & s > 0) 
  # 
