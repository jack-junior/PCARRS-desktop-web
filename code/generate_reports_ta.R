...
data_demo <- read_excel(here("data", "Demo_Participant_Detail_Tamil.xlsx")) %>%
...
"
  ...
  report_out <- file.path(cfg$paths$summaries, "reports_Tamil")
  if(!dir.exists(report_out)) dir.create(report_out, recursive = TRUE)
  
  final_path <- file.path(reports_Tamil, paste0("Report_Tamil_", pid, ".docx"))
...
report_files <- list.files(here("reports_Tamil"), pattern = "Report_Tamil_.*\\.docx", full.names = TRUE)
...
final_combined_path <- here("reports_Tamil", "Full_Study_Combined_Report_tamil.docx")
